...
function makeWord(lettersObject) {
  const lengthArr = Math.max(...Object.keys(lettersObject));
  const arr = Array.from({ length: lengthArr }).fill(0);
...
function sellTickets(queue) {
  const cashObj = { 25: 0, 50: 0, 100: 0 };

  return queue.every((bill) => {
    if (bill === 25) {
      cashObj[25] += 1;
    } else if (bill === 50) {
      if (cashObj[25] === 0) return false;
      cashObj[50] += 1;
      cashObj[25] -= 1;
    } else if (bill === 100) {
      if (cashObj[50] > 0 && cashObj[25] > 0) {
        cashObj[50] -= 1;
        cashObj[25] -= 1;
      } else if (cashObj[25] >= 3) {
        cashObj[25] -= 3;
      } else {
        return false;
      }
